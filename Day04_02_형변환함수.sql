...
/*
    ...
    3. 오라클은 SELECT - FROM절이 필수이므로 언제나 테이블이 필요하다. 
        테이블이 필요없는 값을 조회할 때 DUAL 테이블을 사용한다. 
*/

/* 1. 숫자로 변환하기(TO_NUMBER) */
--'100' - > 100
SELECT '100', TO_NUMBER('100') -- '100'은 문자로도 확인하려고 본 것 
  FROM DUAL;
...
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
  ...
  WHERE EMPLOYEE_ID = '150'; -- EMPLOYEE_ID = TO_NUMBER('150')으로 자동으로 변환한 뒤 처리한다. 
                             -- 숫자와 문자의 연산 (동등비교 연산) TO_NUMBER를 오라클이 자동으로 넣어서 150을 숫자로 변환함. 
...
-- 먼저, 현재 날짜와 시간 확인하기 
SELECT 
        SYSDATE -- DATE 타입의 현재 날짜와 시간 --시간까지 보려면 추가 변경작업 필요 
      , SYSTIMESTAMP -- TIMESTAMP 타입의 현재 날짜와 시간 (나노초 : 10억분의 1초까지 확인 가능) 
   FROM 
        DUAL; 
...
SELECT 
     ...
     , TO_CHAR(DT1, 'YYYY-MM-DD') -- 원하는 형식이 적용 됨. 형식을 바꾸는 건 CHAR
  FROM 
       SAMPLE_TBL;