/*
    ...
    2. SELECT절에서 조회하려는 칼럼은 "반드시" GROUP BY절에 있어야 한다. (필수 규칙) --집계함수 예외 칼럼대상 
...
/*
    ...
    3. GROUP BY 이후에만 처리할 수 있는 조건은 HAVING절이 처리한다. -- 나머지 일반적인 조건은 모두 WHERE절로 처리할 것. 
    
*/

-- 5. DEPARTMENT_ID가 NULL인 부서를 제외하고, 모든 부서의 부서별 사원 수를 조회하시오.
-- 해설 ) DEPARTMENT_ID가 NULL인 부서의 제외는 GROUP BY 이전에 처리할 수 있으므로 WHERE절로 처리한다. 
...
SELECT 
  ...
  GROUP BY
        DEPARTMENT_ID
  HAVING
        COUNT(*) <=5; -- NULL 제외하지 않았으니까 NULL도 나오는 게 맞음
