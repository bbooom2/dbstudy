...
-- 8. 원하는 자릿수로 반올림 
--    ROUND(A,[DIGIT]) : 실수 A를 DIGIT 자릿수로 반올림 -- 이때 [DIGIT] 생략가능한 부분이라 대괄호 처리함, 생략 시 정수로 반올린 값을 반환
SELECT 
...
SELECT 
 ...
 -- 생각해보기 
 -- FLOOR 함수와 TRUNC 함수는 무엇이 다른가? 음수가 들어갈 때 다름 
 -- FLOOR : 작은 정수 찾기 
 -- TRUNC : 보이는 소수점 자르기 
 SELECT 
        FLOOR(1.9)
      , TRUNC(1.9)
      , FLOOR(-1.9) -- -2  보다 작은 정수라고 생각함 
      , TRUNC(-1.9) -- -1  보이는 대로 자름 
  FROM 
        DUAL;    
