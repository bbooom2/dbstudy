...
-- 프로시저1 정의(만들기)
CREATE OR REPLACE PROCEDURE PROC1 --매개변수 없는 버전 볼 것 
IS --변수넣든말든 일단 IS나 AS 넣어야 돌아감 
BEGIN 
    DBMS_OUTPUT.PUT_LINE('Hello World'); --컴파일까지만 됨. 호출 안들어감. 
END; 
...
-- 프로시저3 호출(실행하기)
EXECUTE PROC3(0); -- 이 100이 WHERE로 가고 PROC3 매개변수 


-- 프로시저4 정의(만들기)
-- 사원번호가 100인 사원의 FIRST_NAME, LAST_NAME을 출력 파라미터로 반환하는 프로시저 -반환 리턴을 만든다고 생각 
-- 출력 파라미터 
...
BEGIN 
    ...
    INTO FNAME, LNAME -- 이곳에서 출력 파라미터에 값이 저장된다. 
    FROM EMPLOYEES
    WHERE EMPLOYEE_ID = 100; 
END; 
...
-- 프로시저5. 정의(만들기) : 사원번호를 전달하면 해당 사원의 FIRST_NAME이 출력 파라미터 FNAME에 저장되도록 작성. 
...
EXCEPTION 
    WHEN NO_DATA_FOUND THEN
        FNAME := 'Noname'; --fname에 저장할 값은 노네임 
END; --메소드를 하나 만들었다... 

-- 프로시저5 호출(실행하기) 
DECLARE
     FNAME EMPLOYEES.FIRST_NAME%TYPE; -- 변수가 있어야 호출이 되기 때문 
BEGIN 
    PROC5(0, FNAME); 
...
/*
    ...
    2. 처리할 일 
         ...
         2) 제품 테이블의 재고 내역을 수정한다.(UPDATE)한다. 
...
CREATE TABLE PROD_TBL (
    P_CODE NUMBER NOT NULL, --PK 줄 거라는 얘기 
...
ALTER TABLE PROD_TBL 
    ADD CONSTRAINT PK_PROD1 PRIMARY KEY(P_CODE);
...
-- BUY_PROC 프로시저 정의 
CREATE OR REPLACE PROCEDURE BUY_PROC(CNO IN CUST_TBL.C_NO%TYPE, PCODE IN PROD_TBL.P_CODE%TYPE, BUY_AMOUNT IN BUY_TBL.B_AMOUNT%TYPE) --매개변수가 밑에서 사용됨 
IS
BEGIN
    -- 1) 구매내역 테이블에 구매 내역을 추가(INSERT)한다. 
    ...
    -- CEIL(가격 * 구매수량 * 0.1) 가격은 제품테이블에 있음. 제품테이블에서 가격을 가져오는 셀렉트 구성해야 함.  SELECT * BUY_AMOUNT * 0.1
...
EXCEPTION  --문제가 발생할 수 있으니 예외처리 
 WHEN OTHERS THEN 
    -- 예외 사유 확인 
    DBMS_OUTPUT.PUT_LINE(SQLCODE || '(' || SQLERRM || ')');
    
    -- 롤백 
    ROLLBACK; -- 커밋 이전에 했던 것 취소 
