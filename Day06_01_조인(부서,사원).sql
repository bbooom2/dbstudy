...
/*
    ...
    1. 드라이브(DRIVE) 테이블 
     ...
     3) 행(ROW)의 개수가 일반적으로 적고, PK를 조인 조건으로 사용하기 때문에 인덱스(INDEX) 활용이 가능하다. (PK를 쓸 수 있으면 PK를 쓰는 게 속도 향상에 도움이 된다)
...
-- 3. 오른쪽 외부 조인(오른쪽에 있는 테이블은 일치하는 정보가 없어도 조인한다.)
--1) 표준 문법
SELECT D.DEPT_NO, D.DEPT_NAME, E.EMP_NO, E.NAME
...
SELECT D.DEPT_NO, D.DEPT_NAME, E.EMP_NO, E.NAME
FROM DEPARTMENT_TBL D, EMPLOYEE_TBL E
WHERE D.DEPT_NO (+) = E.DEPART;  --반대편에 (+) 오른쪽외부조인이면 왼쪽에 (+) 표기   
