...
--테이블 삭제 
DROP TABLE EMPLOYEE_TBL; --시퀀스 만들것. 테이블 하나 당 시퀀스 하나 EMP_NO 활용 DEPARTMENT에서 시퀀스 하나 썼기 때문에 다른 테이블인 EMPLOYEE 생성 가능 
...
CREATE SEQUENCE DEPARTMENT_SEQ --이름은 임의로 지으면 됨. 
    ...
    NOCYCLE -- 번호 순환이 없다. (생략 가능)         CYCLE : 번호 MAXVALUE에 도달하면 다음 번호는 MINVALUE이다. 
...
UPDATE EMPLOYEE_TBL
   SET SALARY = SALARY + 500000 --이런 형식으로만 작성 가능 자바처럼 줄여쓰기 불가 
 WHERE DEPART = 1;      -- 조건문(여기서 등호는 비교 연산자이다.)
COMMIT;
