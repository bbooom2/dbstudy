...
/*
    1. SYS, SYSTEM 계정은 (이걸로 접속해야 DROP USER 가능) 관리 계정이므로 해당 계정에서 작업을 하지 않는다. 
    2. 새로운 계정을 만들고, 해당 계정으로 접속해서 작업을 수행한다. 
    3. 새로운 계정을 만드는 작업은 SYS, SYSTEM 계정에서 처리한다. (삭제도 마찬가지)
    4. 새로운 계정을 만드는 방법 
        1) DROP USER 계정이름 CASECADE : 기존에 생성된 계정이 있다면 삭제하시오. CASCADE는 계정이 가진 데이터(테이블)도 함께 삭제하라는 옵션이다. 
