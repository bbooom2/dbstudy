...
/*
    ...
    2. 형식
        ...
        BEGIN
            수행할 PL/SQL
        END; 
    3. 변수에 저장된 값을 출력하기 위해서 서버 출력을 ON 시켜줘야 한다.  -- 시스템 아웃 같은 것 
    최초 1번만 실행시켜 두면 된다. (디폴트는 OFF)
     SET SERVEROUTPUT ON;
*/

-- 서버출력 ON 
SET SERVEROUTPUT ON; --파일 닫거나 접속을 끊기 전까지는 다시 실행 시킬 필요 없음 

-- 서버출력 확인 
BEGIN  
   DBMS_OUTPUT.PUT_LINE('Hello World'); --출력하고 줄바꿈 (PLINTLN과 동일) 
END;
...
3. 복사하는 쿼리 
 ...
 2) 데이터를 제외하고 구조만 복사하기 
    CREATE TABLE 테이블 AS(SELECT 칼럼 FROM 테이블 WHERE1=2); --편견없이 보기 이거 FALSE 만족하는 데이터 없음 그래서 구조만(칼럼만) 복사한다는 것 
...
AS(SELECT * 
    FROM HR.EMPLOYEES); --계정이 다르니까 반드시 명시. 이것도 서브쿼리임(쿼리 안에 쿼리 들어가니까) 
...
1. 대입 연산자(:=)를 사용한다.
2. 종류
    1) 스칼라 변수 : 타입을 직접 지정 한다. 
    2) 참조 변수 : 특정 칼럼에 타입을 가져다가 지정한다. 
...
-- 3. 레코드 변수 
-- 여러 칼럼의 값을 동시에 저장하는 변수 (객체와 느낌 비슷~!) 
...
END;
 ...
 DECLARE
    EMP_ID EMPLOYEES.EMPLOYEE_ID%TYPE;
    SAL EMPLOTYEES.EMPLOYEE_SALARY%TYPE;
    TOTAL NUMBER;
    CNT NUMBER;
...
-- 3) DEPARTMEN_ID가 50이면 행 변수에 저장된 내용을 DEPT50 테이블에 INSERT
...
-- 1부터 3의 정수 값을 누적하시오. 누적 값이 100을 초과하면 그만 누적하고 어디까지 누적했는지 출력하시오. 
DECLARE
     N NUMBER;
     TOTAL NUMBER;
     MODULAR NUMBER(1);
...
BEGIN 
    SELECT FIRST_NAME
     ...
     WHERE EMPLOYEE_ID = 0;
EXCEPTION
    WHEN NO_DATA_FOUND THEN --데이터가 없다 
